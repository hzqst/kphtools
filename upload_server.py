...
try:
    ...
    if 'libcrypto' in error_str or 'LibraryNotFoundError' in error_type:
        ...
        print("=" * 70)
        print("")
        print("This error typically occurs due to one of the following reasons:")
        print("")
        print("1. Missing OpenSSL development libraries:")
        print("   On Ubuntu/Debian:")
        print("     sudo apt-get update")
        print("     sudo apt-get install -y libssl-dev")
        print("")
        print("   On CentOS/RHEL/Fedora:")
        print("     sudo yum install -y openssl-devel")
        print("     # or on newer versions:")
        print("     sudo dnf install -y openssl-devel")
        print("")
        print("2. Outdated oscrypto library (common with OpenSSL 3.x):")
        print("   The oscrypto library may need to be upgraded to support OpenSSL 3.x:")
        print("     pip install --upgrade oscrypto")
        print("     # or upgrade all dependencies:")
        print("     pip install --upgrade -r requirements.txt")
        print("")
        print("3. Python package cache issues:")
        print("   Try clearing Python cache and reinstalling:")
        print("     find . -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null || true")
        print("     pip uninstall -y oscrypto signify")
        print("     pip install --no-cache-dir signify")
        print("")
        print("Diagnostic commands (run these to check your system):")
        print("  openssl version                    # Check OpenSSL version")
        print("  ldconfig -p | grep libcrypto       # Check if libcrypto is available")
        print("  dpkg -l | grep libssl-dev          # Check if libssl-dev is installed (Debian/Ubuntu)")
        print("  pip show oscrypto                  # Check oscrypto version")
        print("")
        print("=" * 70)
    else:
